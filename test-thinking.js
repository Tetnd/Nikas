...
function boostedTokens(thinkingEnabled, effectiveMaxTokens) {
...
check('thinking off + 8K → 8K (no boost)', boostedTokens(false, 8192) === 8192);
check('thinking off + 4K → 4K (no boost)', boostedTokens(false, 4096) === 4096);
check('thinking on + 8K → 16K (boosted)', boostedTokens(true, 8192) === 16384);
check('thinking on + 4K → 16K (boosted)', boostedTokens(true, 4096) === 16384);
check('thinking on + 32K → 32K (already above min)', boostedTokens(true, 32768) === 32768);
check('thinking on + 128K → 128K (unchanged)', boostedTokens(true, 131072) === 131072);

console.log('\n=== 5. End-to-end: effective request shape per effort ===');
function chatRequest(effort, baseMax) {
    return { max_tokens: boostedTokens(effort !== 'off', baseMax), ...buildThinkingParams(effort) };
}
function responsesRequest(effort, baseMax) {
    return { max_output_tokens: boostedTokens(effort !== 'off', baseMax), ...buildResponsesThinkingParams(effort) };
}
...
check('responses max request boosts + enables', rr.max_output_tokens === 16384 && rr.reasoning.effort === 'max');
