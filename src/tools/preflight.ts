...
import {
    ...
    PREFLIGHT_CALL_ID_SEPARATOR,
    PREFLIGHT_TOOL_NAME_HASH_LENGTH,
} from './consts.js';
...
/**
 * Inspect the conversation for provider-owned `activate_*` preflight rounds.
 *
 * When tool-list stabilization is enabled, this extension emits tool calls for
 * VS Code/Copilot's virtual `activate_*` tools so Copilot expands them and the
 * `tools` array stays stable across turns. Those preflight calls live in the
 * replayed history; this inspects what has already been called since the
 * latest human user message.
 *
 * Ported from upstream Vizards/deepseek-v4-for-copilot (tools/preflight.ts,
 * v0.5.1 #77 + #86).
 */
...
/**
 * Remove provider-owned preflight tool call/result parts (plus the empty text
 * parts Copilot leaves behind) from the request history so they are never
 * sent to DeepSeek.
 */
...
/**
 * Provider-owned preflight call ID (provider-safe format, upstream #86).
 *
 * `deepseek_preflight_activate_<round>_<sha256(toolName)[:32]>` — the hashed
 * tool name keeps IDs short and within the conservative alnum/_ set for
...
/**
 * The hashed format no longer encodes the tool name — callers recover it from
 * `LanguageModelToolCallPart.name` (see parsePreflightPart).
 */
